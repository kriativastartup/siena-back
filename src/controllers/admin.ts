...
const SUPER_USER = process.env.SUPER_EMAIL;
const SUPER_PASSWORD = process.env.SUPER_PASSWORD;
const SUPER_FULL_NAME = process.env.SUPER_FULL_NAME || "Admin Principal";


export const create_super_user = async () => {
   
    if (!SUPER_USER || !SUPER_PASSWORD || !SUPER_FULL_NAME) {
        return;
    }
